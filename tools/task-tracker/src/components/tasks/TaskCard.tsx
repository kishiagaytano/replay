...
export function TaskCard({
  ...
  return (
    <article
      ...
      className={cn(
        "group rounded-lg border border-slate-200 border-l-[3px] bg-white p-3 shadow-sm transition-shadow hover:shadow-md",
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <button
          onClick={() => onEdit?.(task)}
          className="text-left text-sm font-medium leading-snug text-slate-900 hover:text-accent"
        >
          {task.title}
        </button>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between gap-2 border-t border-slate-100 pt-2.5">
        <div className="flex items-center gap-1.5">
          ...
          <span className={cn("text-[11px]", overdue ? "font-medium text-red-600" : "text-slate-400")}>
        ...
        {onStatusChange ? (
          <select
            ...
            className="rounded-md border border-slate-200 bg-white px-1.5 py-1 text-[11px] text-slate-600 focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent/40"
          >
            {STATUS_LIST.map((s) => (
